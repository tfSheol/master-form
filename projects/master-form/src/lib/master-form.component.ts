  ...
  implements OnDestroy, OnInit, AfterContentInit
{
  ...
  @Output() onInit?: EventEmitter<MasterFormHelperData<any>> =
    new EventEmitter();
  @Output() onData?: EventEmitter<MasterFormHelperData<any>> =
    new EventEmitter();
  @Output() onValidate?: EventEmitter<boolean> = new EventEmitter();
  @Output() onError?: EventEmitter<any[]> = new EventEmitter();
  @Output() onObservableUpdate?: EventEmitter<MasterFormHelperData<any>> =
    new EventEmitter();
  ...
  ngAfterContentInit() {
    this.addSubscription(
      ...
      this.clear.subscribe(() => {
        ...
        this.subscriptions
          .filter((sub) => sub.type == SubType.DEFAULT)
    ...
    this.addSubscription(
      SubType.SYSTEM,
      this.render.subscribe(() => this.renderItem())
    );
    this.addSubscription(
      SubType.SYSTEM,
  ...
  private addInput<T>(masterForm: MasterForm): MasterFormComponent {
    this.formGroup.addControl(
      masterForm.name,
      new FormControl('', masterForm.controls)
    );
    return this;
  }
  ...
  public selectPlaceholder(masterForm: MasterForm): string {
    return !masterForm.placeholder
      ? masterForm.display
      : masterForm.placeholder;
  }
    ...
    if (masterForms.length > 0) {
      masterForms?.forEach((masterForm: MasterForm) => {
        ...
        if (masterForm.data) {
          this.addSubscription(
            SubType.DEFAULT,
            masterForm.data().subscribe((data) => {
          ...
          if (masterForm.hasFilter) {
            ...
            this.addSubscription(
              SubType.DEFAULT,
              filterCtrl.valueChanges.subscribe((search) => {
          ...
          masterForms.forEach((el) => {
            if (el.customDataField) {
              ...
              data?.forEach((element) => {
                let vl = el.value?.find(
                  (e) => e[el.customDataField.sourceField] === element
                );
                if (vl) {
  ...
  ngOnInit(): void {
    ...
    if (this.observable) {
      this.addSubscription(
        SubType.SYSTEM,
        this.observable.subscribe((masterForms) => {
  ...
  private filter(masterForm: MasterForm, search: string) {
    ...
    if (masterForm.customDataField) {
      masterForm.filteredValue = masterForm.value.filter(
        (item) =>
          item[masterForm.customDataField.sourceField]
            .toLowerCase()
            .indexOf(search) > -1
      );
    } else {
